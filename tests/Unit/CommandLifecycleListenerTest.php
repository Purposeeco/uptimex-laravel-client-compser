...
it('records a command event with name + exit code on Finished', function () {
    ...
    $listener->onStarting(new CommandStarting('schedule:work', new ArgvInput(['app']), new BufferedOutput));
    expect(Uptimex::context()?->type)->toBe('command');

    $listener->onFinished(new CommandFinished('schedule:work', new ArgvInput(['app']), new BufferedOutput, 0));
    ...
    expect($event['type'])->toBe('command')
        ->and($event['name'])->toBe('schedule:work')
...
it('skips its own commands so they can flush inline', function () {
    $listener = $this->app->make(CommandLifecycleListener::class);
    $listener->onStarting(new CommandStarting('uptimex:test', new ArgvInput(['app']), new BufferedOutput));
